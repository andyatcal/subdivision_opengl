...
A mesh also includes the adjacency and connectivity of elements in it. They can be classifed into three types: 1) halfedge flow in an individual face, 2) face connections, and 3) boundary connections.
...
\subsection{Build Mesh from Elements}

In order to construct a mesh from basic elements, we need four steps.

Step1: create individual vertices. We create instances of vertices, with their position and ID defined. The position of a vetex can be the same but the ID should always be unique. This step taks O(V) time, where V is the number of vertices in the mesh.

Step2: construct indivdual face. To build an indivdual face, we create consecutive instances of halfedges, with their start and end vertices given. We also add the 
