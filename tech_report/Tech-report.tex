...
With definition extension of mobius connection, there can be three different situations for a halfedge in a mesh: 1) it has a sibling pointer and on a normal connection, 2) it has a mobius pointer and on a mobius connection, and 3) it does not have a sibling pointer or mobius poniter, and it lies on the boundary of the surface.

\subsection{Boundaries}

For halfedges on boundary of the mesh, they contains boundary links to each other. In a surface with no mobius connetion, the boundary halfedges follows a continous flow. It can start from one vertex and end at this vertex eventually by following the flow on the boundary. We create previous and next boundary pointers to these halfedges. However, when mobius connection occurs in the mesh, some adjacent boundary halfedges start or end at same vertex. We build mobius boundary pointers to these halfedges. Figure X shows an example of boundary halfedges with and without mobius connections.
